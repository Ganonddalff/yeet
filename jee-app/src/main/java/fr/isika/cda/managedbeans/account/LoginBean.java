	...
	public String login() {
		...
		if (optional.isPresent()) {
			...
			if (account.getPassword().equals(loginForm.getPassword())
					&& account.getIdentifier().equals(loginForm.getIdentifier())) {
				...
				if (account.getAccountCategory() == AccountCategory.User
						|| account.getAccountCategory() == AccountCategory.Administrator)
				...
				FacesContext.getCurrentInstance().addMessage(formulaire.getClientId(), new FacesMessage(
						"L'identifiant et/ou le mot de passe ne correspondent pas à un compte existant"));
			}
		} else {
			UIComponent formulaire = FacesContext.getCurrentInstance().getViewRoot().findComponent("loginForm");
